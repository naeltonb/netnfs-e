        ...
        Try
            ...
            Select Case _servico
                ...
                Case WsNfse.TipoServico.RECEPCAO_PROCESSAMENTO_LOTE_RPS
                    ...
                    Dim strCabecalhoWebService As String = CabecalhoWebService()
                ...
                Case WsNfse.TipoServico.CONSULTA_LOTE_RPS
                    ...
                    End If
                        ...
                        'xmlRetorno.Load("C:\Users\Josimar\ValidaXML\retorno.xml")
...
#Region "Metodos privados"
        ...
        If xmlDoc.GetElementsByTagName("ListaNfse").Count = 0 Then 'Retornou Msg Erro
            ...
            If xmlDoc.GetElementsByTagName("ListaMensagemRetornoLote").Count <> 0 Then
                ...
                For Each infMensagem As XmlElement In nodeListaMensagem
                    ...
                    Dim nodeIdentificacaoRps As XmlNodeList = infMensagem.GetElementsByTagName("IdentificacaoRps").Item(0).ChildNodes
                    For Each infIdentRps As XmlElement In nodeIdentificacaoRps
                        retorno.NumRps = "Rps: " & infIdentRps.InnerText & "-"
                    Next
    ...
    Private Sub InserirTabelaLote(ByVal _numLote As Integer, ByVal _status As Integer, ByVal _protocolo As String, ByVal _dataProtocolo As String, ByVal _listRetorno As List(Of Retorno))
        ...
        Try
            ...
            'FECHAR TRANSAÇÃO DE BANCO DE DADOS AQUI
        Catch ex As Exception
    ...
    Private Sub InserirNotasNoBanco(ByVal _listaInfNfse As List(Of InfNfse))
        ...
        Try
            ...
            For Each _infNfse As InfNfse In _listaInfNfse
                ...
                "Intermediario,OrgaoGerador,CodigoVerificacao,ValorServico,CodigoObra) values" & _
                ...
                "'" & _infNfse.CodObra & "')"
                conBd.ExecutarSqlNfse(Sql, _infNfse.DataEmissao, _infNfse.Competencia)
    ...
    'Retorna o Cabeçalho de envio WebService do Municipio correspondente
    Private Function CabecalhoWebService() As String

        'Pega a URL do template de acordo com o Municipio de emissão do contribuinte
        Dim strDiretorioTemplate As String
        Select Case MunicipioEmissao
            Case MunicipioDeEmissao.BeloHorizonte
                strDiretorioTemplate = My.Application.Info.DirectoryPath & "\Templates\BH\CabecalhoWebService.xml"
            Case MunicipioDeEmissao.Contagem
                strDiretorioTemplate = My.Application.Info.DirectoryPath & "\Templates\Contagem\CabecalhoWebService.xml"
            Case MunicipioDeEmissao.RibeiraoDasNeves
                strDiretorioTemplate = My.Application.Info.DirectoryPath & "\Templates\RibeiraoDasNeves\CabecalhoWebService.xml"
            Case Else
                strDiretorioTemplate = ""
        End Select

        'Carrega o xml e converte em string
        Dim xmlCabecalho As New XmlDocument
        xmlCabecalho.PreserveWhitespace = False
        xmlCabecalho.Load(strDiretorioTemplate)
        Dim strCabecalho As String = Funcoes.XmlDocumentToString(xmlCabecalho)

        'Destroi as variáveis utilizadas
        xmlCabecalho = Nothing
        strDiretorioTemplate = Nothing
